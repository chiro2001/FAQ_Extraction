...
class QuestionGeneration(AutoRegressiveDecoder):
    ...
    def generate(self, answer, topk=1):
...
class Evaluator(keras.callbacks.Callback):
    ...
    def evaluate(self, data, topk):
        ...
        for a, q in tqdm(data):
            total += 1
            q = ' '.join(q)
            pred_q = ' '.join(self.qg.generate(a, topk))
            if pred_q.strip():
...
def do_train():
    data = load_data('./data/train.csv')  # 加载数据
    ...
    for fold, (trn_idx, val_idx) in enumerate(kf.split(data), 1):
        ...
        train_model.load_weights(f'./saved_model/baseline-sougou-fold-1.h5')
        ...
        callbacks = [
            ...
            EarlyStopping(
                ...
                mode='max'),
            ModelCheckpoint(
                f'./saved_model/baseline-sougou-zhaoshang-fold-{fold}.h5',
                monitor='val_rouge_l',
                save_weights_only=True,
                save_best_only=True,
                verbose=1,
                mode='max'),
            TensorBoard(log_dir='./output/zhaoshang_logs', histogram_freq=0, batch_size=batch_size, write_graph=True,
                        write_grads=False,
        ...
        predict_to_file(valid_data, f'./prediction_result/qa_zhaoshang_predict-{fold}.csv', qg)
